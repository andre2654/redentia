<!--
  EditorNotification — editor dedicado para comunicações do tipo
  `notification`.

  Diferente de banner/cta/modal (intrusivos), notification e PASSIVO:
  aparece num bell icon no header com badge de unread, abre num
  drawer (`InboxDrawer`) quando user clica no sino. Ideal pra:
    - aviso "1:N do admin pra users especificos"
    - pop sem interromper navegacao
    - persistente ate user marcar como lida

  Caracteristicas chave:
    - placement = 'inbox' sempre (auto-set)
    - sem modal_size — renderiza em formato fixo de drawer item
    - title curto + body curto (sem rich text — vira preview text)
    - icon opcional (Lucide) destaca categoria
    - link_url + link_label (clique no item leva pra essa URL)
    - dismissible: user marca como lida → remove do unread badge
    - frequency: persistente. Ate user marcar. Sem session gate.

  Sections:
    1. Mensagem      — title + body curto + icon
    2. Acao          — link de destino + label do botao
    3. Audiencia     — base + escopo + refinamentos
    4. Vigencia      — timeline + presets + priority
-->
<template>
  <div class="notification-editor">
    <!-- LEFT: form -->
    <form class="notification-editor__form" @submit.prevent="$emit('save')">
      <!-- =================== STATUS ROW =================== -->
      <div class="status-row">
        <div class="status-row__intro">
          <span class="status-row__eyebrow">Status atual</span>
          <span class="status-row__hint">
            Notificação aparece no <strong>sino do header</strong>. Persiste até o user marcar como lida.
          </span>
        </div>
      ...
      >
        <Field label="Título *" hint="Manchete do item no drawer. Curto e claro.">
          <UInput
            v-model="form.title"
            placeholder='Ex.: "Sua corretora foi reconectada"'
            size="md"
            required
          />
          <span class="field-counter" :class="{ 'field-counter--warn': (form.title || '').length > 70 }">
        ...
        <Field
          ...
          hint="Texto preview que aparece abaixo do título. 1-2 linhas. Plain text, sem formatação."
        >
          <UTextarea
            v-model="form.body"
            :rows="3"
            :maxlength="280"
            placeholder='Ex.: "Conexão Pluggy renovada. Próxima sincronização: hoje às 18h."'
            size="md"
          />
          <span class="field-counter" :class="{ 'field-counter--warn': (form.body || '').length > 240 }">
            {{ (form.body || '').length }}/280
          </span>
        </Field>

        <Field
          label="Ícone"
          hint="Pictograma que diferencia a categoria. Aparece à esquerda do título no drawer."
        >
      ...
      >
        ...
        <Field label="Link de destino" hint="URL absoluta ou path interno.">
          <div class="cta-url-input">
            ...
            <UInput
              ...
              placeholder="/wallet, /pricing, https://..."
              size="md"
        ...
        <Field label="Texto do link (opcional)" hint='Ex.: "Ver detalhes", "Conectar agora". Vazio = clique direto no card.'>
          <UInput
        ...
        <div class="aud-layer">
          <div class="aud-layer__head">
            ...
            <div>
              ...
              <span class="aud-layer__sub">Quem recebe a notificação?</span>
            </div>
          </div>
          ...
          <div v-if="form.target_audience === 'specific'" class="aud-layer aud-layer--accent">
            <div class="aud-layer__head">
              ...
              <div>
                ...
                <span class="aud-layer__sub">Lista manual de destinatários.</span>
              </div>
            ...
            <div v-if="!selectedUserIds.length" class="aud-warn">
              ...
              Sem destinatários selecionados, ninguém vai receber.
            </div>
          </div>
        </Transition>
      ...
      >
        <span class="section-lead">
          Quando a notificação <strong>deve aparecer</strong> e <strong>expirar</strong>.
          Vazio em fim = persiste indefinidamente até user marcar como lida.
        </span>
        ...
        <div class="grid-2">
          ...
          <Field label="Fim" hint="Vazio = persiste até user marcar como lida.">
            <div class="vig-input">
        ...
        <Field label="Prioridade" hint="Notificações de maior prioridade aparecem primeiro no drawer.">
    ...
    <aside class="notification-editor__aside">
      <!-- Preview do drawer com bell + item -->
      <div class="preview-card">
        <header class="preview-card__head">
          <span class="preview-card__eyebrow">Preview no inbox</span>
          <span class="preview-card__hint">Header sino + drawer item</span>
        </header>

        <div class="preview-stage preview-stage--inbox">
          <!-- Header simulado com bell + badge -->
          <div class="prev-inbox-header">
            <span class="prev-inbox-bell">
              <UIcon name="i-lucide-bell" class="size-4" />
              <span class="prev-inbox-bell__badge">1</span>
            </span>
            <span class="prev-inbox-arrow">
              <UIcon name="i-lucide-arrow-down-right" class="size-3" />
            </span>
          </div>

          <!-- Drawer item -->
          <div class="prev-inbox-drawer">
            <div class="prev-inbox-drawer__head">
              <span class="prev-inbox-drawer__title">Notificações</span>
              <span class="prev-inbox-drawer__hint">1 nova</span>
            </div>

            <article class="prev-inbox-item">
              <span class="prev-inbox-item__dot" />
              <span class="prev-inbox-item__icon">
                <UIcon
                  :name="form.icon || 'i-lucide-bell'"
                  class="size-4"
                />
              </span>
              <div class="prev-inbox-item__body">
                <span class="prev-inbox-item__title">{{ form.title || '(Título da notificação)' }}</span>
                <span v-if="form.body" class="prev-inbox-item__text">{{ form.body }}</span>
                <span v-if="form.link_url || form.link_label" class="prev-inbox-item__link">
                  <UIcon :name="linkType.icon" class="size-3" />
                  {{ form.link_label || 'Abrir' }}
                  <code v-if="form.link_url">{{ shortLink }}</code>
                </span>
                <span class="prev-inbox-item__time">agora</span>
              </div>
              <button type="button" class="prev-inbox-item__close" tabindex="-1" aria-label="Marcar como lida">
                <UIcon name="i-lucide-check" class="size-3" />
              </button>
            </article>
          </div>
      ...
      <div v-if="analytics" class="analytics-card">
        ...
        <div class="analytics-grid">
          <div class="analytics-stat">
            <span class="analytics-stat__label">Entregues</span>
            <span class="analytics-stat__value">{{ analytics.impressions }}</span>
            <span class="analytics-stat__sub">{{ analytics.unique_seen }} viram</span>
          </div>
          <div class="analytics-stat">
          ...
          <div class="analytics-stat">
            <span class="analytics-stat__label">Lidas</span>
            <span class="analytics-stat__value">{{ analytics.dismissals }}</span>
            <span class="analytics-stat__sub">{{ analytics.unique_dismissed }} usuários</span>
          </div>
        </div>
      </div>
    </aside>
...
// Auto-set placement = 'inbox' (notification renderiza pelo InboxDrawer).
watch(() => props.form.placement, (next) => {
  if (next !== 'inbox') props.form.placement = 'inbox'
}, { immediate: true })
...
const statusChoices: { value: CommunicationStatus; label: string; help: string }[] = [
  { value: 'draft', label: 'Rascunho', help: 'Não envia. Use enquanto edita.' },
  { value: 'active', label: 'Ativo', help: 'Notificação aparece no sino dos destinatários.' },
...
// =================================================================
// CTA helpers (link detector + suggestions)
// =================================================================
...
const urlSuggestions = [
  { value: '/wallet', label: '/wallet', icon: 'i-lucide-wallet', desc: 'Carteira' },
  { value: '/settings/integracoes', label: '/settings/integracoes', icon: 'i-lucide-link', desc: 'Conexões' },
  { value: '/help', label: '/help', icon: 'i-lucide-message-circle', desc: 'Chat' },
  { value: '/raio-x', label: '/raio-x', icon: 'i-lucide-scan-line', desc: 'Raio-X' },
  { value: '/pricing', label: '/pricing', icon: 'i-lucide-credit-card', desc: 'Planos' },
]

const shortLink = computed(() => {
  const u = props.form.link_url || ''
  if (!u) return ''
...
const durationLabel = computed<string>(() => {
  ...
  const start = props.form.starts_at ? new Date(props.form.starts_at).getTime() : null
  const end = props.form.ends_at ? new Date(props.form.ends_at).getTime() : null
  if (!start && !end) return 'Persiste até ser lida'
...
const schedulePresets: SchedPreset[] = [
  { id: 'now-24h', label: 'Próximas 24h', icon: 'i-lucide-clock', desc: 'Inicia agora, expira em 24h' },
  { id: 'now-7d', label: '7 dias', icon: 'i-lucide-calendar', desc: 'Inicia agora, expira em 7 dias' },
  { id: 'now-30d', label: '30 dias', icon: 'i-lucide-calendar-days', desc: 'Inicia agora, expira em 30 dias' },
  { id: 'tomorrow', label: 'Amanhã 9h', icon: 'i-lucide-sunrise', desc: 'Programa pra começar amanhã às 9h' },
  { id: 'unlimited', label: 'Indefinido', icon: 'i-lucide-infinity', desc: 'Persiste até user marcar como lida' },
]
...
const priorityPresets: PriorityPreset[] = [
  { value: 0, kind: 'low', label: 'Baixa', desc: 'Aparece abaixo de outras' },
  { value: 100, kind: 'medium', label: 'Média', desc: 'Default' },
  { value: 500, kind: 'high', label: 'Alta', desc: 'Topo do drawer' },
  { value: 1000, kind: 'urgent', label: 'Urgente', desc: 'Sempre primeiro' },
]
...
.notification-editor__form {
  display: flex;
  flex-direction: column;
  gap: 14px;
  min-width: 0;
}
.notification-editor__aside {
  display: flex;
  flex-direction: column;
  gap: 14px;
  position: sticky;
...
/* =========================================================
   CTA url input + suggestions (compartilhado com EditorCta/Modal
   mas mantido inline aqui pra autonomia da componente)
   ========================================================= */
.cta-url-input {
  display: flex;
  align-items: center;
...
.prev-inbox-bell__badge {
  ...
  font-weight: 700;
  padding: 0 4px;
  border: 2px solid color-mix(in srgb, var(--brand-text) 3%, transparent);
}
.prev-inbox-arrow {
  color: var(--brand-primary);
  animation: prev-bounce 1.6s ease-in-out infinite;
}
@keyframes prev-bounce {
...
.prev-inbox-drawer__title {
  font-family: var(--brand-font);
  font-size: 13px;
  font-weight: 500;
  color: var(--brand-text);
}
.prev-inbox-drawer__hint {
  font-family: 'JetBrains Mono', monospace;
  font-size: 9.5px;
  font-weight: 600;
...
.prev-inbox-item {
  display: grid;
  grid-template-columns: 6px 32px 1fr 22px;
  align-items: flex-start;
  gap: 10px;
  padding: 12px 14px;
  position: relative;
}
.prev-inbox-item__dot {
  width: 6px;
  height: 6px;
  border-radius: 999px;
  background: var(--brand-primary);
...
.prev-inbox-item__body {
  display: flex;
  flex-direction: column;
  gap: 2px;
  min-width: 0;
}
.prev-inbox-item__title {
  font-family: var(--brand-font);
  font-size: 13px;
  font-weight: 500;
  color: var(--brand-text);
...
.prev-inbox-item__link code {
  font-family: 'JetBrains Mono', monospace;
  font-size: 9.5px;
  color: color-mix(in srgb, var(--brand-text) 65%, transparent);
}
.prev-inbox-item__time {
  font-family: 'JetBrains Mono', monospace;
  font-size: 9.5px;
  color: color-mix(in srgb, var(--brand-text) 45%, transparent);
  margin-top: 4px;
}
.prev-inbox-item__close {
  display: inline-flex;
  align-items: center;
  justify-content: center;
  width: 22px;
