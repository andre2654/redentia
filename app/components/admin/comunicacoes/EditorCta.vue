<!--
  EditorCta — editor dedicado pra comunicações do tipo `cta`.

  Por que separado: CTA é diferente de banner (faixa fina) e de
  anúncio (artigo no feed). É um card contextual que aparece em
  páginas-alvo específicas (topo da carteira, das calculadoras,
  rankings, guias, etc) com chamada pra acao clara.

  Modos:
    - "Conteúdo + CTA": title + body rich + icon + link_label/url
    - "Imagem clicável": só image_url + link_url (a imagem inteira
      vira o link, sem texto). Pra promocoes visuais, parcerias,
      campanhas com criativo já prontas.

  Placements suportados:
    - Legado: wallet-hero, help-prompt, home-cta
    - Novos: calculadora-top, ranking-top, guias-top,
      dividendos-top, asset-top, tesouro-top, crypto-top,
      setor-top, pricing-top, glossario-top

  Workflow: ongoing — KEEPS active/paused (CTA roda enquanto
  campanha ativa).

  Sections:
    1. Modo (toggle)
    2. Conteúdo (varia conforme modo)
    3. Onde aparece (placement picker categorizado)
    4. Audiência (layered builder)
    5. Vigência (timeline + presets + priority)
-->
<template>
  <div class="cta-editor">
    <!-- LEFT: form -->
    <form class="cta-editor__form" @submit.prevent="$emit('save')">
      <!-- =================== STATUS ROW =================== -->
      <div class="status-row">
        <div class="status-row__intro">
          <span class="status-row__eyebrow">Status atual</span>
          <span class="status-row__hint">CTA é uma campanha contínua. Liga e desliga conforme estratégia.</span>
        </div>
      ...
      <Section
        ...
        :open="open.mode"
        @toggle="open.mode = !open.mode"
      >
        <span class="section-lead">
          Escolha como o CTA é apresentado: <strong>conteúdo editorial</strong> (título + texto + botão) ou
          <strong>imagem clicável</strong> (criativo pronto, banner promocional).
        </span>

        <div class="cta-mode-grid">
          <button
            type="button"
            class="cta-mode-card"
            :class="{ 'cta-mode-card--active': mode === 'content' }"
            @click="setMode('content')"
          >
            <span class="cta-mode-card__icon">
              <UIcon name="i-lucide-text" class="size-5" />
            </span>
            <span class="cta-mode-card__title">Conteúdo + CTA</span>
            <span class="cta-mode-card__desc">
              Título + corpo + ícone + botão. Pra explicar a oferta antes de chamar pra ação.
            </span>
            <span v-if="mode === 'content'" class="cta-mode-card__check">
          ...
          <button
            type="button"
            class="cta-mode-card"
            :class="{ 'cta-mode-card--active': mode === 'image' }"
            @click="setMode('image')"
          >
            <span class="cta-mode-card__icon">
              <UIcon name="i-lucide-image" class="size-5" />
            </span>
            <span class="cta-mode-card__title">Imagem clicável</span>
            <span class="cta-mode-card__desc">
              Só uma imagem que leva ao link. Pra criativo pronto, banner de parceria, promoção visual.
            </span>
            <span v-if="mode === 'image'" class="cta-mode-card__check">
              <UIcon name="i-lucide-check" class="size-3" />
            </span>
          </button>
        </div>
      ...
      <!-- =================== SECTION 2: CONTEÚDO =================== -->
      <Section
        :title="mode === 'image' ? 'Imagem' : 'Mensagem'"
        eyebrow="Conteúdo"
      ...
      >
        ...
        <template v-if="mode === 'image'">
          <Field
            label="Imagem *"
            hint="A imagem inteira vira clicável. Recomendado 16:9 ou 21:9 (banner widescreen)."
          >
            <AdminFormsImageUploadField v-model="form.image_url" :alt="form.title || ''" />
          </Field>

          <Field
            label="Texto descritivo (alt)"
            hint="Descrição da imagem pra acessibilidade e SEO. Não aparece na UI."
          >
            <UInput
              v-model="form.title"
        ...
        <!-- Modo CONTEÚDO -->
        <template v-else>
          <Field label="Título *" hint="Manchete do CTA. Curto e direto.">
            <UInput
              v-model="form.title"
          ...
          >
            <AdminFormsRichTextField
              ...
              level="basic"
              :max-length="500"
              placeholder='Ex.: "Sincronize automaticamente sua carteira via Open Finance e atualize o raio-X em tempo real."'
            />
          </Field>

          <div class="grid-2">
            <Field label="Ícone" hint="Pictograma ao lado do título.">
              <AdminFormsIconPicker v-model="form.icon" />
            </Field>
            <Field label="Imagem cover (opcional)" hint="Aparece acima do título no preview se preenchida.">
              <AdminFormsImageUploadField v-model="form.image_url" :alt="form.title || ''" />
            </Field>
          </div>
        </template>
      ...
      >
        <span class="section-lead">
          {{ mode === 'image'
            ? 'A imagem inteira vira clicável e leva pra esse destino.'
            : 'URL pra onde o botão leva. Detectamos automaticamente o tipo.' }}
        </span>

        <Field
          :label="mode === 'image' ? 'Link de destino *' : 'Link de destino'"
          hint="URL absoluta ou path interno."
        >
        ...
        <Field
          ...
          hint="Imperativo curto. Ex: 'Acessar', 'Conectar', 'Ver detalhes'."
        >
          <UInput
        ...
        <Field label="Dispensável" hint="Mostra um X que permite o usuário fechar o CTA.">
          <label class="switch">
            <input v-model="form.dismissible" type="checkbox" />
            <span class="switch__track" />
            <span class="switch__label">{{ form.dismissible ? 'Usuário pode fechar' : 'Permanente (sem X)' }}</span>
          </label>
        </Field>
      </Section>

      <!-- =================== SECTION 4: ONDE APARECE (placements multi) =================== -->
      <Section
        title="Onde aparece"
      ...
      >
        <div class="placement-multi-head">
          <span class="section-lead">
            Selecione <strong>um ou mais lugares</strong> onde o CTA aparece.
            Mesma campanha em vários slots = 1 set de analytics agregado.
          </span>
          <div class="placement-multi-summary">
            <span class="placement-multi-summary__count" :data-state="selectedPlacements.length ? 'filled' : 'empty'">
              {{ selectedPlacements.length }}
              <span class="placement-multi-summary__count-label">
                {{ selectedPlacements.length === 1 ? 'lugar' : 'lugares' }}
              </span>
            </span>
            <button
              v-if="selectedPlacements.length"
              type="button"
              class="placement-multi-summary__clear"
        ...
        <div class="cta-placement-groups">
          <div v-for="group in placementGroups" :key="group.label" class="cta-placement-group">
            <div class="cta-placement-group__head">
              <span class="cta-placement-group__label">{{ group.label }}</span>
              <button
                v-if="groupHasAnyActive(group)"
                type="button"
                class="cta-placement-group__action"
            ...
            <div class="cta-placement-grid">
              <button
                v-for="p in group.items"
                :key="p.value"
                type="button"
                class="cta-placement-card"
                :class="{ 'cta-placement-card--active': isPlacementActive(p.value) }"
                :title="p.desc"
                @click="togglePlacement(p.value)"
              >
                <span class="cta-placement-card__checkbox" :class="{ 'cta-placement-card__checkbox--active': isPlacementActive(p.value) }">
                  <UIcon v-if="isPlacementActive(p.value)" name="i-lucide-check" class="size-3" />
                </span>
                <span class="cta-placement-card__icon">
                  <UIcon :name="p.icon" class="size-4" />
                </span>
                <div class="cta-placement-card__meta">
                  <span class="cta-placement-card__label">{{ p.label }}</span>
                  <span class="cta-placement-card__desc">{{ p.desc }}</span>
                </div>
              </button>
            </div>
          </div>
          ...
          <div v-if="form.target_audience === 'specific'" class="aud-layer aud-layer--accent">
            <div class="aud-layer__head">
              ...
              <div>
                ...
                <span class="aud-layer__sub">Lista manual de usuários que recebem o CTA.</span>
              </div>
            ...
            <div v-if="!selectedUserIds.length" class="aud-warn">
              <UIcon name="i-lucide-alert-triangle" class="size-3.5" />
              Sem usuários selecionados, ninguém vai ver esse CTA.
            </div>
          </div>
        </Transition>
      ...
      >
        <span class="section-lead">
          Quando o CTA deve <strong>começar</strong> e <strong>terminar</strong>. Vazio = sem limite.
        </span>
        ...
        <Field label="Prioridade" hint="Quando há vários CTAs ativos no mesmo placement, o de maior prioridade aparece.">
    ...
    <aside class="cta-editor__aside">
      <!-- PREVIEW -->
      <div class="preview-card">
        <header class="preview-card__head">
          <span class="preview-card__eyebrow">Preview ao vivo</span>
          <span class="preview-card__hint">{{ placementHint }}</span>
        </header>

        <div class="preview-stage">
          <!-- Modo IMAGEM: imagem inteira clicável -->
          <div v-if="mode === 'image'" class="prev-cta-image">
            <div v-if="form.image_url && !imgErrored" class="prev-cta-image__wrap">
              <img
                :src="form.image_url"
                :alt="form.title || ''"
                class="prev-cta-image__img"
                @error="onImgError"
              />
              <span v-if="form.dismissible" class="prev-cta-image__close">
                <UIcon name="i-lucide-x" class="size-3" />
              </span>
            </div>
            <div v-else class="prev-cta-image__placeholder">
              <UIcon name="i-lucide-image" class="size-6" />
              <span>Cole a URL da imagem pra ver o preview</span>
            </div>
            <div v-if="form.link_url" class="prev-cta-image__destination">
              <UIcon :name="linkType.icon" class="size-3" />
              <span class="prev-cta-image__destination-label">leva pra</span>
              <code>{{ form.link_url }}</code>
            </div>
          </div>

          <!-- Modo CONTEÚDO: card editorial -->
          <div v-else class="prev-cta-card">
            <span class="prev-cta-card__glow" />
            <img
              v-if="form.image_url && !imgErrored"
              :src="form.image_url"
              alt=""
              class="prev-cta-card__img"
              @error="onImgError"
            />
            <header class="prev-cta-card__head">
              <UIcon
                v-if="form.icon"
                :name="form.icon"
                class="prev-cta-card__icon size-5"
              />
              <h3 class="prev-cta-card__title">{{ form.title || '(Título do CTA)' }}</h3>
              <span v-if="form.dismissible" class="prev-cta-card__close">
                <UIcon name="i-lucide-x" class="size-3.5" />
              </span>
            </header>
            <div v-if="form.body" class="prev-cta-card__body" v-html="renderedBody" />
            <span v-if="form.link_url || form.link_label" class="prev-cta-card__btn">
...
// =================================================================
// Mode toggle: 'content' (title+body+icon+button) | 'image' (only image+link)
// Persisted via convention: se image_url tem valor mas title/body sao
// nulls, é "image" mode. Caso contrario "content".
// =================================================================
type Mode = 'content' | 'image'
const mode = ref<Mode>(detectInitialMode())

function detectInitialMode(): Mode {
  // Inicial = 'image' se image_url setado E body vazio
  if (props.form.image_url && !props.form.body) return 'image'
  return 'content'
}

function setMode(next: Mode) {
  mode.value = next
  // Limpa campos não-relevantes pro modo (mas NAO limpa o image_url
  // que pode ser cover na contentmode)
  if (next === 'image') {
    // image-only: zerar body/icon/link_label (mas NAO title — vira alt-text)
    props.form.body = null
    props.form.icon = null
    props.form.link_label = null
  }
}
...
const statusChoices: { value: CommunicationStatus; label: string; help: string }[] = [
  ...
  { value: 'active', label: 'Ativo', help: 'CTA entra na página alvo pra audiência configurada.' },
  { value: 'paused', label: 'Pausado', help: 'Removido temporariamente (sem perder analytics).' },
]

interface PlacementMeta {
  value: CommunicationPlacement
  label: string
  desc: string
  icon: string
}
...
const placementGroups: PlacementGroup[] = [
  ...
  },
  {
    label: 'Páginas-alvo',
    items: [
      { value: 'asset-top', label: 'Topo de ativo', desc: '/asset/{ticker} no topo', icon: 'i-lucide-line-chart' },
      { value: 'calculadora-top', label: 'Topo das calculadoras', desc: '/calculadora/* no topo', icon: 'i-lucide-calculator' },
      { value: 'ranking-top', label: 'Topo dos rankings', desc: '/ranking/* no topo', icon: 'i-lucide-bar-chart-3' },
      { value: 'guias-top', label: 'Topo dos guias', desc: '/guias/* no topo', icon: 'i-lucide-book-open' },
      { value: 'dividendos-top', label: 'Topo de dividendos', desc: '/dividendos/* no topo', icon: 'i-lucide-coins' },
      { value: 'tesouro-top', label: 'Topo do Tesouro', desc: '/tesouro/* no topo', icon: 'i-lucide-banknote' },
      { value: 'crypto-top', label: 'Topo de Cripto', desc: '/crypto/* no topo', icon: 'i-lucide-bitcoin' },
      { value: 'setor-top', label: 'Topo de Setor', desc: '/setor/* no topo', icon: 'i-lucide-pie-chart' },
      { value: 'glossario-top', label: 'Topo do Glossário', desc: '/glossario/* no topo', icon: 'i-lucide-book' },
    ],
  },
  {
    label: 'Públicas',
    items: [
      { value: 'home-cta', label: 'Home pública', desc: '/ no destaque público', icon: 'i-lucide-home' },
      { value: 'pricing-top', label: 'Topo de Pricing', desc: '/pricing no topo', icon: 'i-lucide-credit-card' },
    ],
  },
]
...
/**
 * Multi-placement state — array de placements onde o CTA aparece. Tem
 * fonte de verdade no `form.placements` (jsonb no backend). Pra compat
 * com placements antigos que so populam o single `form.placement`,
 * inicializa o array a partir dele se vier vazio.
 */
const selectedPlacements = computed<CommunicationPlacement[]>(() => {
  if (Array.isArray(props.form.placements) && props.form.placements.length) {
    return props.form.placements as CommunicationPlacement[]
  }
  // Fallback pro single placement (banner-style ou CTA criado antes do multi).
  if (props.form.placement) return [props.form.placement]
  return []
})

function isPlacementActive(value: CommunicationPlacement): boolean {
  return selectedPlacements.value.includes(value)
}

function togglePlacement(value: CommunicationPlacement): void {
  const current = [...selectedPlacements.value]
  const idx = current.indexOf(value)
  if (idx >= 0) current.splice(idx, 1)
  else current.push(value)
  applyPlacements(current)
}

function toggleGroup(group: PlacementGroup, select: boolean): void {
  const current = new Set(selectedPlacements.value)
  for (const item of group.items) {
    if (select) current.add(item.value)
    else current.delete(item.value)
  }
  applyPlacements(Array.from(current))
}

function groupHasAnyActive(group: PlacementGroup): boolean {
  return group.items.some(item => selectedPlacements.value.includes(item.value))
}
...
/**
 * Sync entre `form.placements` (array, fonte) e `form.placement` (single,
 * legacy). Backend faz o mesmo no save mas mantemos o single em sync
 * client-side pra que outras leituras (ex.: placementHint) batam.
 */
function applyPlacements(next: CommunicationPlacement[]): void {
  const dedup = Array.from(new Set(next))
  ;(props.form as any).placements = dedup.length ? dedup : null
  ;(props.form as any).placement = dedup[0] ?? null
}

const placementHint = computed(() => {
  const count = selectedPlacements.value.length
  if (!count) return 'Sem placement'
  if (count === 1) {
    const flat = placementGroups.flatMap(g => g.items)
    return flat.find(p => p.value === selectedPlacements.value[0])?.label || 'Sem placement'
  }
...
const priorityPresets: PriorityPreset[] = [
  { value: 0, kind: 'low', label: 'Baixa', desc: 'Cede vez pra outros CTAs' },
...
// =================================================================
// Sub-components Section + Field (mesma pattern)
// =================================================================
...
/* =========================================================
   Audience layered builder (base → escopo → refinamentos)
   ========================================================= */
.aud-layer {
  position: relative;
  display: flex;
...
/* =========================================================
   CTA-specific: mode toggle + image mode + placement groups
   + preview de imagem clicável + URL detector
   ========================================================= */
...
/* Mode toggle (Conteúdo + CTA / Imagem clicável) */
.cta-mode-grid {
  display: grid;
  grid-template-columns: 1fr;
  gap: 10px;
...
@media (min-width: 640px) {
  .cta-mode-grid { grid-template-columns: 1fr 1fr; }
}
.cta-mode-card {
  position: relative;
  display: flex;
...
.cta-mode-card--active .cta-mode-card__icon {
  background: color-mix(in srgb, var(--brand-primary) 22%, transparent);
  color: var(--brand-primary);
}
.cta-mode-card__title {
  font-family: var(--brand-font);
  font-size: 13.5px;
  font-weight: 600;
  letter-spacing: -0.005em;
...
.cta-image-input:focus-within {
  border-color: var(--brand-primary);
  box-shadow: 0 0 0 3px color-mix(in srgb, var(--brand-primary) 18%, transparent);
}
.cta-image-input__icon { color: color-mix(in srgb, var(--brand-text) 50%, transparent); flex-shrink: 0; }
.cta-image-input__field { flex: 1; }
.cta-image-input__field :deep(input) {
  border: 0 !important;
  background: transparent !important;
  box-shadow: none !important;
  padding-left: 0 !important;
}
.cta-image-input__clear {
  display: inline-flex;
  align-items: center;
  justify-content: center;
  width: 22px;
...
.cta-image-preview__img { width: 100%; height: 100%; object-fit: cover; }
.cta-image-preview__error {
  display: inline-flex;
  align-items: center;
  gap: 6px;
  padding: 6px 10px;
...
.cta-placement-group__head {
  display: flex;
  align-items: center;
  justify-content: space-between;
  gap: 12px;
}
.cta-placement-group__label {
  font-family: 'JetBrains Mono', monospace;
  font-size: 9.5px;
  font-weight: 700;
  letter-spacing: 0.18em;
...
.cta-placement-group__action:hover {
  color: var(--brand-primary);
}

/* Head row da section: lead + summary com contador + botao limpar */
.placement-multi-head {
  display: flex;
  align-items: flex-start;
  justify-content: space-between;
  gap: 16px;
...
.placement-multi-summary {
  display: inline-flex;
  align-items: center;
  gap: 8px;
  flex-shrink: 0;
}
.placement-multi-summary__count {
  display: inline-flex;
  flex-direction: column;
  align-items: center;
  padding: 4px 10px;
...
.placement-multi-summary__count[data-state='empty'] {
  border-color: color-mix(in srgb, var(--brand-warning) 35%, transparent);
  background: color-mix(in srgb, var(--brand-warning) 12%, transparent);
  color: var(--brand-warning);
}
.placement-multi-summary__count-label {
  font-family: 'JetBrains Mono', monospace;
  font-size: 8.5px;
  font-weight: 600;
  letter-spacing: 0.14em;
...
.placement-multi-summary__clear {
  display: inline-flex;
  align-items: center;
  gap: 4px;
  padding: 5px 10px;
...
.placement-multi-summary__clear:hover {
  background: color-mix(in srgb, var(--brand-negative) 10%, transparent);
  color: var(--brand-negative);
  border-color: color-mix(in srgb, var(--brand-negative) 35%, transparent);
}

.placement-multi-warn {
  display: inline-flex;
  align-items: center;
  gap: 6px;
  padding: 7px 10px;
...
@media (min-width: 720px) {
  .cta-placement-grid { grid-template-columns: 1fr 1fr; }
}
.cta-placement-card {
  position: relative;
  display: flex;
...
.cta-placement-card:hover {
  border-color: color-mix(in srgb, var(--brand-text) 18%, transparent);
  background: color-mix(in srgb, var(--brand-text) 5%, transparent);
}
.cta-placement-card--active {
  border-color: color-mix(in srgb, var(--brand-primary) 50%, transparent);
  background: color-mix(in srgb, var(--brand-primary) 10%, transparent);
}
.cta-placement-card__icon {
  display: inline-flex;
  align-items: center;
  justify-content: center;
  width: 26px;
...
.cta-placement-card--active .cta-placement-card__icon {
  background: color-mix(in srgb, var(--brand-primary) 22%, transparent);
  color: var(--brand-primary);
}
.cta-placement-card__meta { display: flex; flex-direction: column; gap: 1px; flex: 1; min-width: 0; }
.cta-placement-card__label {
  font-family: var(--brand-font);
  font-size: 12px;
  font-weight: 500;
  color: var(--brand-text);
}
.cta-placement-card__desc {
  font-family: 'JetBrains Mono', monospace;
  font-size: 10px;
  color: color-mix(in srgb, var(--brand-text) 50%, transparent);
}
.cta-placement-card__check {
  width: 16px;
  height: 16px;
  display: inline-flex;
  align-items: center;
...
/* Checkbox-style indicator pra multi-select. Diferente do `__check`
   (circulo solido pra single-select), e um quadrado vazio que vira
   solido com checkmark quando selecionado. Visual hint: "voce pode
   marcar varios". */
.cta-placement-card__checkbox {
  width: 16px;
  height: 16px;
  display: inline-flex;
  align-items: center;
...
.cta-placement-card__checkbox--active {
  background: var(--brand-primary);
  border-color: var(--brand-primary);
}
.cta-placement-card:hover .cta-placement-card__checkbox:not(.cta-placement-card__checkbox--active) {
  border-color: color-mix(in srgb, var(--brand-text) 45%, transparent);
}

/* Preview ao vivo */
.preview-stage { padding: 14px; background: rgba(0, 0, 0, 0.2); min-height: 220px; }
...
.prev-cta-image__wrap {
  position: relative;
  border-radius: 10px;
  overflow: hidden;
  box-shadow: 0 8px 24px -10px color-mix(in srgb, var(--brand-primary) 30%, transparent);
  cursor: pointer;
}
.prev-cta-image__img {
  width: 100%;
...
.prev-cta-image__wrap:hover .prev-cta-image__img {
  transform: scale(1.015);
}
.prev-cta-image__close {
  position: absolute;
  top: 8px;
  right: 8px;
  width: 22px;
...
.prev-cta-image__destination-label {
  font-family: 'JetBrains Mono', monospace;
  font-size: 9px;
  font-weight: 600;
  letter-spacing: 0.16em;
...
.prev-cta-image__destination code {
  font-family: 'JetBrains Mono', monospace;
  font-size: 11px;
  color: var(--brand-text);
...
.prev-cta-card__icon { color: var(--brand-primary); }
.prev-cta-card__title {
  flex: 1;
  margin: 0;
  font-family: var(--brand-font);
  font-size: 14.5px;
...
.prev-cta-card__body :deep(p) { margin: 0 0 6px; }
.prev-cta-card__body :deep(p:last-child) { margin-bottom: 0; }
.prev-cta-card__body :deep(strong) { font-weight: 600; color: var(--brand-text); }
.prev-cta-card__body :deep(em) {
  font-style: italic;
  font-family: 'Instrument Serif', Georgia, serif;
}
.prev-cta-card__body :deep(a) {
  color: var(--brand-primary);
  text-decoration: underline;
}
.prev-cta-card__body :deep(ul),
.prev-cta-card__body :deep(ol) { margin: 4px 0; padding-left: 18px; }
.prev-cta-card__btn {
  display: inline-flex;
  align-items: center;
  gap: 6px;
