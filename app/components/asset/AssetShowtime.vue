  ...
  >
    ...
    <div class="relative mx-auto w-full max-w-6xl px-6 pb-10 pt-8 md:grid md:grid-cols-12 md:gap-10 md:px-10 md:pb-14 md:pt-12">
      ...
      <div class="pointer-events-none absolute inset-0 opacity-25" aria-hidden="true">
        ...
        <div class="absolute right-[30%] top-[55%] size-2 rounded-full" :style="{ backgroundColor: 'var(--brand-positive)' }" />
      </div>
      ...
      <div class="relative md:col-span-7">
        ...
        >
          A ficha da<br />
          <span class="highlighter" :style="{ color: 'var(--brand-primary)' }">{{ assetName || tickerUpper }}</span>
        </h1>

        <p v-if="!isLoadingAsset && asset?.sector" class="font-showtime-body mt-5 max-w-xl text-base" :style="{ color: 'color-mix(in srgb, var(--brand-text) 80%, transparent)' }">
          Setor: <strong :style="{ color: 'var(--brand-primary)' }">{{ asset.sector }}</strong>{{ asset?.industry ? ' · ' + asset.industry : '' }}. Tudo que você precisa saber desse bicho num lugar só, criatura, sem jargão, sem cara feia.
        </p>
        ...
        <div class="mt-8 flex flex-wrap items-baseline gap-x-5 gap-y-2">
          ...
          <span
            ...
            :style="{
              backgroundColor: Number(asset?.change_percent) >= 0 ? `${'var(--brand-positive)'}25` : `${'var(--brand-negative)'}25`,
        ...
        >
          ...
          <dl class="mt-5 flex flex-col gap-3">
            <div v-for="row in showtimeQuickPanel" :key="row.label" class="flex items-center justify-between">
              <dt class="font-showtime-label !text-[11px] flex items-center gap-2" :style="{ color: 'color-mix(in srgb, var(--brand-text) 50%, transparent)' }">
                <UIcon :name="row.icon" class="size-3.5" :style="{ color: 'var(--brand-primary)' }" />
                {{ row.label }}
              </dt>
    ...
    <div v-if="mascotImage" class="mx-auto w-full max-w-6xl px-6 pb-8 md:px-10">
      <div
        ...
        :style="{ borderColor: `${'var(--brand-primary)'}60`, backgroundColor: `color-mix(in srgb, var(--brand-primary) 6%, transparent)` }"
      >
        <img :src="mascotImage" alt="Mascote" class="h-14 shrink-0" />
        <p class="font-showtime-body text-sm italic" :style="{ color: 'color-mix(in srgb, var(--brand-text) 80%, transparent)' }">
          <strong :style="{ color: 'var(--brand-primary)' }">{{ mascotName }} diz:</strong>
        ...
        <div class="mb-8 flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            >
              ...
              <span class="highlighter" :style="{ color: 'var(--brand-primary)' }">{{ tickerUpper }}</span>
            </h2>
          </div>
    ...
    <div v-if="mascotImage" class="flex items-center justify-center gap-6 py-6">
      <div class="h-[2px] max-w-[180px] flex-1" :style="{ backgroundColor: `color-mix(in srgb, var(--brand-primary) 33%, transparent)` }" />
      ...
      <div class="mx-auto w-full max-w-6xl px-6 md:px-10">
        <div class="mb-10 text-center">
          ...
          >
            ...
            <span class="highlighter" :style="{ color: 'var(--brand-primary)' }">pra gente normal</span>
          </h2>
          <p class="font-showtime-body mx-auto mt-6 max-w-2xl text-base" :style="{ color: 'color-mix(in srgb, var(--brand-text) 80%, transparent)' }">
        ...
        <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          ...
          >
            ...
            <div
              ...
              :style="{ backgroundColor: `${'var(--brand-primary)'}25` }"
            >
              <UIcon :name="card.icon" class="size-6" :style="{ color: 'var(--brand-primary)' }" />
            </div>
            <span class="font-showtime-label" :style="{ color: 'color-mix(in srgb, var(--brand-text) 50%, transparent)' }">
              {{ card.label }}
            </span>
            ...
            <p class="font-showtime-body mt-3 !text-[12px] italic" :style="{ color: 'color-mix(in srgb, var(--brand-text) 70%, transparent)' }">
      ...
      <div class="mx-auto w-full max-w-6xl px-6 md:px-10">
        <div class="grid gap-10 md:grid-cols-12">
          <div class="md:col-span-5">
            ...
            >
              Afinal, o que a<br />
              <span class="highlighter" :style="{ color: 'var(--brand-primary)' }">{{ assetName }}</span> faz?
            </h2>
            <p class="font-showtime-body mt-5 text-base" :style="{ color: 'color-mix(in srgb, var(--brand-text) 80%, transparent)' }">
          ...
          <div class="md:col-span-7">
            ...
            >
              <div class="washi-tape" />
              <p class="font-showtime-body text-[15px] leading-relaxed" :style="{ color: 'color-mix(in srgb, var(--brand-text) 90%, transparent)' }">
                {{ asset.description }}
              </p>
            </div>
          </div>
