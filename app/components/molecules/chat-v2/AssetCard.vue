...
<template>
  ...
  >
    ...
    <div class="flex min-w-0 flex-1 flex-col leading-tight">
      <span class="font-mono-tab text-[14px] font-semibold tracking-tight" :style="{ color: brand.colors.text }">
        {{ card.ticker }}
      </span>
      <span class="line-clamp-1 text-[11.5px]" :style="{ color: brand.colors.textMuted }">
        {{ name }}
      </span>
    </div>

    <div class="flex flex-col items-end gap-0.5 text-right leading-tight">
      <span v-if="price" class="font-mono-tab text-[13px] font-semibold tabular-nums" :style="{ color: brand.colors.text }">
        {{ price }}
      </span>
