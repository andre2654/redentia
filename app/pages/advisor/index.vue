  ...
  <NuxtLayout title="Área do assessor">
    <div class="flex h-full flex-col gap-6 p-6">
      <div class="flex flex-col gap-2">
        <h2 class="text-[18px] font-medium" :style="{ color: 'var(--brand-text)', letterSpacing: '-0.005em' }">Meus assessorados</h2>
        <p class="text-[13px]" :style="{ color: `color-mix(in srgb, var(--brand-text) 70%, transparent)` }">
      ...
      <div v-else-if="!investors.length" class="rounded-xl border border-white/10 bg-white/5 px-6 py-12 text-center" :style="{ color: `color-mix(in srgb, var(--brand-text) 60%, transparent)` }">
      ...
      <div v-else class="flex flex-col gap-3">
        ...
        >
          <NuxtLink :to="`/advisor/investors/${inv.id}`" class="flex min-w-0 flex-1 flex-col gap-1">
            <span class="font-medium" :style="{ color: 'var(--brand-text)' }">{{ inv.name }}</span>
            <span class="text-xs" :style="{ color: `color-mix(in srgb, var(--brand-text) 50%, transparent)` }">{{ inv.email }}</span>
            <span v-if="inv.last_contact_at" class="text-[11px]" :style="{ color: `color-mix(in srgb, var(--brand-text) 40%, transparent)` }">
