    ...
    <main class="mx-main">
      ...
      <template v-else-if="activeTab === 'Início'">
        ...
        <section class="mx-hero-d mx-show-d">
          ...
          <p class="mx-hero-sub">
            Sua carteira fechou em <strong class="mx-pos">+R$ 1.840</strong> ontem. Tudo no controle.
          </p>
        </section>

        <!-- Saldo (mobile mostra inline, desktop dentro do grid 3-col) -->
        <section class="mx-section mx-section-conta mx-hide-d">
          <p class="mx-label">Conta</p>
          <button class="mx-balance-row" @click="setTab('Carteira')">
            <span v-if="showBalance" class="mx-balance">R$ 142.380<span class="mx-cents">,45</span></span>
            <span v-else class="mx-balance-hidden" />
            <UIcon name="i-lucide-chevron-right" class="size-5 mx-balance-chevron" />
          </button>
        </section>

        <!-- Chips peach -->
        <div class="mx-chips mx-hide-d">
          <button v-for="c in quickChips" :key="c.label" class="mx-chip">
        ...
        <!-- Service tiles row -->
        <section class="mx-tiles-section">
          <h3 class="mx-section-h mx-show-d">Acesso rápido</h3>
          <div class="mx-tiles">
            <button v-for="s in services" :key="s.label" class="mx-tile">
              <UIcon :name="s.icon" class="size-6 mx-tile-icon" />
              <span class="mx-tile-label">{{ s.label }}</span>
            </button>
          </div>
        </section>

        <!-- DESKTOP 3-COL: Saldo amber / Notificações compact / Raio-X dark -->
        <section class="mx-grid mx-grid-3 mx-show-d">
          <!-- Saldo card amber (desktop only) -->
          <article class="mx-saldo-card">
            <p class="mx-saldo-eyebrow">Conta</p>
            <p class="mx-saldo-val">R$ 142.380<span class="mx-saldo-cents">,45</span></p>
            <p class="mx-saldo-delta">
              <UIcon name="i-lucide-arrow-up" class="size-3.5" />
              <strong>R$ 8.420</strong>
              <span class="mx-saldo-mut">de rendimento · 12m</span>
            </p>
            <button class="mx-saldo-cta" @click="setTab('Carteira')">
              Ver carteira
              <UIcon name="i-lucide-arrow-right" class="size-4" />
            </button>
          </article>

          <!-- Notificações compact -->
          <article class="mx-card">
            <header class="mx-card-head">
              <h3 class="mx-card-title">Notificações</h3>
              <button class="mx-list-link">Ver todas</button>
            </header>
            <ul class="mx-notif-list-d">
              <li v-for="n in notifications.slice(0, 4)" :key="n.id" class="mx-notif-row-d">
                <div>
                  <p class="mx-notif-title-d">{{ n.title }}</p>
                  <p class="mx-notif-desc-d">{{ n.description }}</p>
                </div>
                <p class="mx-notif-date-d">{{ n.date }}</p>
              </li>
            </ul>
          </article>

          <!-- Raio-X dark feature -->
          <article class="mx-feature-dark">
            <span class="mx-feature-dark-tag">Análise gratuita</span>
            <h3 class="mx-feature-dark-title">Raio-X da <em>carteira</em>.</h3>
            <p class="mx-feature-dark-body">7 dimensões em 2 minutos.</p>
            <button class="mx-feature-dark-cta">
              Gerar Raio-X
              <UIcon name="i-lucide-arrow-right" class="size-4" />
            </button>
            <div class="mx-feature-dark-glow" aria-hidden="true" />
          </article>
        </section>

        <!-- DESKTOP 2-COL: Investimentos + Descubra -->
        <section class="mx-grid mx-grid-2 mx-show-d">
          <article class="mx-card">
            <header class="mx-card-head">
              <h3 class="mx-card-title">Investimentos</h3>
              <button class="mx-list-link" @click="setTab('Carteira')">Ver carteira →</button>
            </header>
            <div class="mx-invest-d-row">
              <div class="mx-donut-wrap">
                <svg class="mx-donut" viewBox="0 0 200 200">
                  <circle cx="100" cy="100" r="78" fill="none" stroke="var(--bg-overlay)" stroke-width="28" />
                  <circle v-for="(seg, i) in donutSegments" :key="i"
                          cx="100" cy="100" r="78" fill="none"
                          :stroke="seg.color" stroke-width="28"
                          :stroke-dasharray="`${seg.length} ${490 - seg.length}`"
                          :stroke-dashoffset="-seg.offset"
                          transform="rotate(-90 100 100)" />
                </svg>
                <div class="mx-donut-center">
                  <p class="mx-donut-label">Total</p>
                  <p class="mx-donut-val">R$ 142k</p>
                </div>
              </div>
          ...
          <article class="mx-card">
            <h3 class="mx-card-title">Descubra mais</h3>
            <ul class="mx-discover-d-list">
              <li v-for="d in discover" :key="d.title" class="mx-discover-d-row">
                <span class="mx-discover-d-icon" :style="{ background: d.bg, color: d.accent }">
                  <UIcon :name="d.icon" class="size-5" />
                </span>
                <div class="mx-discover-d-meta">
                  <p class="mx-discover-d-t">{{ d.title }}</p>
                  <p class="mx-discover-d-b">{{ d.body }}</p>
                </div>
                <UIcon name="i-lucide-arrow-right" class="size-4 mx-discover-d-arr" />
              </li>
            </ul>
          </article>
        </section>

        <!-- MOBILE: Investimentos mini donut preview -->
        <section class="mx-section mx-hide-d">
          <article class="mx-invest-preview" @click="setTab('Carteira')">
            <div class="mx-invest-donut-wrap">
              <svg class="mx-invest-donut" viewBox="0 0 100 100" aria-hidden="true">
            ...
            <div class="mx-invest-meta">
              <p class="mx-invest-label">Investimentos</p>
              <p class="mx-invest-val">R$ 142.380,45</p>
              <p class="mx-invest-sub">5 classes · <span class="mx-pos">+R$ 8.420 (12m)</span></p>
            </div>
            <UIcon name="i-lucide-chevron-right" class="size-5 mx-invest-chev" />
          </article>
        </section>

        <!-- MOBILE: Notificações carousel -->
        <section class="mx-section-flush mx-hide-d">
          <header class="mx-list-head mx-pad-x">
            <h2 class="mx-section-h2 mx-list-h">Notificações da Redentia</h2>
            <button class="mx-list-link">Ver todas</button>
          </header>
          <div class="mx-notif-carousel">
            <article v-for="n in notifications" :key="n.id" class="mx-notif-card">
              <header class="mx-notif-card-head">
                <p class="mx-notif-card-title">{{ n.title }}</p>
                <p class="mx-notif-card-date">{{ n.date }}</p>
              </header>
              <p class="mx-notif-card-desc">{{ n.description }}</p>
            </article>
          </div>
        </section>

        <!-- MOBILE: Feature Raio-X dark -->
        <section class="mx-section mx-hide-d">
          <article class="mx-feature-dark">
            <span class="mx-feature-dark-tag">Análise gratuita</span>
            <h3 class="mx-feature-dark-title">Raio-X da <em>carteira</em>.</h3>
            <p class="mx-feature-dark-body">
              Descubra os riscos escondidos da sua carteira em 2 minutos. Sete dimensões de análise.
            </p>
            <button class="mx-feature-dark-cta">
              Gerar Raio-X
              <UIcon name="i-lucide-arrow-right" class="size-4" />
        ...
        <!-- Redent Pro feature WIDE (responsive — single col mobile, 2-col desktop) -->
        <section class="mx-section">
          <article class="mx-feature-orange">
            <div class="mx-feature-orange-meta">
              <span class="mx-feature-orange-tag">Redent Pro</span>
              <h3 class="mx-feature-orange-title">Análise institucional ao seu alcance.</h3>
              <p class="mx-feature-orange-body">
                Sem anúncios, raio-x avançado, alertas em tempo real e análises da equipe Redentia.
              </p>
              <button class="mx-feature-orange-cta">
                Conhecer
                <UIcon name="i-lucide-arrow-right" class="size-4" />
              </button>
            </div>
            <ul class="mx-feature-orange-list mx-show-d">
              <li><UIcon name="i-lucide-check" class="size-4" /> Raio-X mensal automático</li>
              <li><UIcon name="i-lucide-check" class="size-4" /> Alertas em tempo real</li>
              <li><UIcon name="i-lucide-check" class="size-4" /> Análises da equipe Redentia</li>
              <li><UIcon name="i-lucide-check" class="size-4" /> Sem anúncios</li>
            </ul>
          </article>
        </section>

        <!-- MOBILE: Descubra mais carousel -->
        <section class="mx-section-flush mx-hide-d">
          <h2 class="mx-section-h2 mx-pad-x">Descubra mais</h2>
          <div class="mx-discover">
            <article v-for="d in discover" :key="d.title" class="mx-discover-card">
              <div class="mx-discover-thumb" :style="{ background: d.bg }">
                <UIcon :name="d.icon" class="size-7" :style="{ color: d.accent }" />
              </div>
              <p class="mx-discover-title">{{ d.title }}</p>
              <p class="mx-discover-body">{{ d.body }}</p>
            </article>
          </div>
        </section>
      </template>
