<!--
  ...
  Refactor: substitui useWalletMockData() pelos services reais
  (usePortfolioTradesService + usePluggyService). Mantem o template
  intacto, mapeando MockTrade (do backend) -> shape WalletTrade que
  o template consome. Toggle de modo agora vem do useResultMode()
  (compartilhado com /wallet/resultado), com mapping pequeno:
...
/**
 * V8 Resultado — versao com dados reais.
 *
 * Substitui useWalletMockData() pelos services reais:
 ...
 *
 * Toggle de modo (swing/day) usa useResultMode() do `/wallet/resultado`
 * (compartilhado). Mapping: 'swing' ↔ 'carteira', 'day' ↔ 'day-trade'.
 *
...
// O template foi escrito assumindo `WalletTrade` (date/qty/price/volume/
// result/style/enterTime/exitTime). MockTrade do backend tem campos
// equivalentes mas com nomes diferentes — mapeamos pra evitar reescrever
// 600 linhas de template.
