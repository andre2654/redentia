import type { IChartConfig, IChartDataPoint, ChartTimeRange } from '~/types/chart';

export const useChartData = () => {
  // Mock data generator para simular dados reais
  const generateMockPriceData = (ticker: string, days: number = 30): IChartDataPoint[] => {
  ...
  const formatChartData = (priceData: IChartDataPoint[], ticker: string): IChartConfig => {
  ...
  const getChartConfig = (ticker: string, timeRange: ChartTimeRange = 'month'): IChartConfig => {
