...
export const formatMarketData = (type: string, rawData: any) => {
    ...
    if (type === 'price') {
        ...
        const lastUpdate = data.updated_at ?? data.updatedAt ?? new Date().toISOString()
    ...
    if (type === 'dividends') {
        ...
        return divs.slice(0, 50).map((d: any) => ({
            date: d.paymentDate || d.date || d.approved_date || d.payment_date || d.last_date_prior,
    ...
    if (type === 'analysis') {
        ...
        if (data.key_statistics) {
            if (!pe) pe = data.key_statistics.forward_pe || data.key_statistics.trailing_eps
